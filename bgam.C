...
Bool_t bgam::Process(Long64_t entry)
{
   ...
   // // Implanted ion map
   // if(FrsHitData_fwr_t.GetSize() > 0 && AidaImplantHits_Time.GetSize() > 0){

   //    for (int i =0; i < FrsHitData_fwr_t.GetSize(); i++)
   //    {
   //       for(int j=0; j < AidaImplantHits_Time.GetSize(); j++){
   //          if(filled_aida_implants_166Tb.count(j) == 0){
   //             if(Tb_x2_Z->IsInside(FrsHitData_fID_z.At(i), FrsHitData_fID_x2.At(i)) && Tb_Z_AoQ->IsInside(FrsHitData_fID_AoQ_corr.At(i), FrsHitData_fID_z.At(i))) // currently set for 166Tb
   //             {
   //                if(filled_frs.count(i) == 0){
   ...
   //                {
   //                   h2_aida1_166Tb_implants->Fill(AidaImplantHits_StripX[j], AidaImplantHits_StripY[j]);
   //                }
   //                if(AidaImplantHits_DSSD[j] == 2)
   //                {
   //                   h2_aida2_166Tb_implants->Fill(AidaImplantHits_StripX[j], AidaImplantHits_StripY[j]);
   //                }

   //             }
   //          }
   //          filled_aida_implants_166Tb.insert(j);
   //          if(filled_aida_implants_160Eu.count(j)==0){
   //             if(Eu_Z_Z2->IsInside(FrsHitData_fID_z2.At(i), FrsHitData_fID_z.At(i)) && Eu_Z_AoQ->IsInside(FrsHitData_fID_AoQ_corr.At(i), FrsHitData_fID_z.At(i))) // currently set for 160Eu
   //             {
   //                if(filled_frs.count(i) == 0){
   //                if(TMath::Abs(FrsHitData_fID_z.At(i)-FrsHitData_fID_z2.At(i)< 0.4)) frs_zvsaoq->Fill( FrsHitData_fID_AoQ_corr.At(i),FrsHitData_fID_z.At(i));
   //                frs_z1vsz2->Fill(FrsHitData_fID_z2.At(i), FrsHitData_fID_z.At(i));
   ...
   //                if(AidaImplantHits_DSSD[j] == 1 && AidaImplantHits_Stopped[j] == true)
   //                {
   //                   h2_aida1_160Eu_implants->Fill(AidaImplantHits_StripX[j], AidaImplantHits_StripY[j]);
   //                }
   //                if(AidaImplantHits_DSSD[j] == 2)
   //                {
   //                   h2_aida2_160Eu_implants->Fill(AidaImplantHits_StripX[j], AidaImplantHits_StripY[j]);
   //                }

   //             }
   //          }
   //          filled_aida_implants_160Eu.insert(j);
   //          filled_frs.insert(i);
